...
type modals =
  | No_modals
  | Modes of mode loc list
  | Modalities of modality loc list

let empty_modals = function
...
and fmt_payload c ctx pld =
  ...
  | PSig ({psg_modalities; _} as mty) ->
      fmt ":"
      $ fmt_modals ~break:false ~ats:`Two c (Modalities psg_modalities)
      $ fmt "@ "
...
and fmt_modals ?(break = true) ~ats c modals =
  let modals =
    match modals with
    | No_modals -> []
    | Modes modes ->
        List.map modes ~f:(fun {txt= Mode mode; loc} -> {txt= mode; loc})
  ...
  in
  fmt_if_k
    (not (List.is_empty modals))
    (fmt_or break "@ " " " $ fmt_ats $ hvbox 0 (list modals "@ " fmt_modal))
...
and fmt_arrow_param ~return c ctx
  ...
  in
  let modes = fmt_modals c (Modes mI) ~ats:`One in
...
and fmt_pattern ?ext c ?pro ?parens ?(box = false)
  ...
  | Ppat_constraint (_, None, []) -> assert false
  | Ppat_constraint (pat, maybe_typ, modes) ->
      let fmt_typ =
        ...
        | Some typ ->
            ...
            $ fmt_core_type c (sub_typ ~ctx typ)
      in
      let ats = if Option.is_some maybe_typ then `Two else `One in
      hvbox 2
        (Params.parens_if parens c.conf
           ( fmt_pattern c (sub_pat ~ctx pat)
           $ fmt_typ
  ...
  | Pexp_letmodule (name, modes, args, pmod, exp) ->
      ...
      let xbody, xmty, xmodes =
        ...
        | { pmod_desc= Pmod_constraint (body_me, body_mt, body_mm)
          ...
          ; pmod_attributes= [] } ->
            let after =
              match List.last body_mm with
              | Some last -> last.loc
              | None -> (Option.value_exn body_mt).pmty_loc
            in
            Cmts.relocate c.cmts ~src:pmod_loc ~before:body_me.pmod_loc
              ~after ;
            ( sub_mod ~ctx body_me
            , Option.map body_mt ~f:(sub_mty ~ctx)
            , body_mm )
        | _ -> (xbody, None, [])
      in
      let can_sparse =
      ...
      $ hvbox 0
          ( Params.parens_if
              ...
              c.conf
              ( hvbox 2
                  (fmt_module c ctx keyword ~eqty:":" name (Modes modes)
                     args (Some xbody) xmty xmodes
...
and fmt_class_field_kind c ctx = function
  ...
  | Cfk_concrete
      ...
      let rec cleanup names e args' =
        match (e, args') with
        | {pexp_desc= Pexp_constraint (e, Some t, []); _}, [] ->
  ...
  | Cfk_concrete (_, e) ->
      let ty, e =
        match e with
        | {pexp_desc= Pexp_constraint (e, Some t, []); _} -> (Some t, e)
        | _ -> (None, e)
      in
      ( opt ty (fun t -> fmt "@ : " $ fmt_core_type c (sub_typ ~ctx t))
      , noop
...
and fmt_label_declaration c ctx ?(last = false) decl =
  ...
  let cmt_after_type =
    Cmts.fmt_after c
      ( match List.last pld_modalities with
      | None -> pld_type.ptyp_loc
      | Some last -> last.loc )
  in
  let field_loose =
    match c.conf.fmt_opts.field_space.v with
  ...
  | Psig_include
      ...
      hvbox 0
        ...
        $ hvbox 0
            ...
            $ esp $ fmt_opt epi
            $ fmt_item_attributes c ~pre:(Break (1, 0)) atrs
            $ fmt_modals ~break:has_attrs c ~ats:`Two (Modalities modalities)
            )
        $ doc_after )
...
and fmt_module c ctx ?rec_ ?epi ?(can_sparse = false) keyword ?(eqty = "=")
    name modals xargs xbody xmty xmodes ~attrs ~rec_flag =
  let ext = attrs.attrs_extension in
  let blk_t =
    match xmty, xmodes with
    | None, [] -> empty
    | None, _ :: _ ->
      { empty with psp= fmt_modals c ~ats:`One (Modes xmodes) }
    | Some xmty, xmodes ->
      let blk = fmt_module_type ?rec_ c xmty in
      { blk with
        pro=
          Some (str " " $ str eqty $ opt blk.pro (fun pro -> str " " $ pro))
      ; psp= fmt_if (Option.is_none blk.pro) "@;<1 2>" $ blk.psp $ fmt_modals c ~ats:`Two (Modes xmodes) }
  in
  ...
  let intro =
    ...
    $ wrap_if
        ( (not (empty_modals modals))
        && ((not (List.is_empty xargs)) || Option.is_some xmty) )
        "(" ")"
...
and fmt_module_expr ?(dock_struct = true) c ({ast= m; _} as xmod) =
  ...
  | Pmod_constraint (me, mt, mm) ->
      ...
      let blk_t =
        match mt with
        | Some mt -> fmt_module_type c (sub_mty ~ctx mt)
        | None -> empty
      in
      let has_epi =
            ...
            $ ( match mt with
              ...
              | Some mt ->
                  ...
                  $ hvbox 0
                      ...
                      $ fmt_opt blk_t.epi ) )
            $ fmt_modals c
                ~ats:(if Option.is_some mt then `Two else `One)
                (Modes mm) )
...
and fmt_value_binding c ~rec_flag ?(punned_in_output = false) ?ext ?in_ ?epi
  ...
  in
  let has_args = not (List.is_empty lb_args) in
  let has_cstr = Option.is_some lb_typ in
  let has_modes = not (List.is_empty lb_modes) in
  fmt_docstring c ~epi:(fmt "@\n") doc1
  $ cmts_before
  $ hvbox 0
      ( hvbox indent
  ...
  let xbody, xmty, xmodes =
    ...
    | { pmod_desc= Pmod_constraint (body_me, body_mt, body_mm)
      ...
      ; pmod_attributes= [] } ->
        ...
        (sub_mod ~ctx body_me, Option.map ~f:(sub_mty ~ctx) body_mt, body_mm)
    | _ -> (xbody, None, [])
  in
  Cmts.fmt c pmb.pmb_loc
    (fmt_module ~rec_:rec_flag c ctx keyword ~rec_flag:(rec_flag && first)
       ~eqty:":" pmb_name (Modes pmb.pmb_modes) pmb.pmb_args (Some xbody) xmty xmodes
       ~attrs )
