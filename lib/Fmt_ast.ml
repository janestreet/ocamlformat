  ...
  let fmt_fun_arg (a : Sugar.arg_kind) =
    ...
    | Val
      ...
      when String.equal l txt ->
        cbox 0
          (wrap "?(" ")"
             ( fmt_if islocal "local_ " $ fmt_pattern c xpat $ fmt " =@;<1 2>"
    ...
    | Val
      ...
      when String.equal l txt ->
        cbox 0
          (wrap "?(" ")"
             ( fmt_if islocal "local_ " $ fmt_pattern c ~parens:false xpat
    ...
    | Val (islocal, Optional l, xpat, Some xexp) ->
        ...
        cbox 2
          ( str "?" $ str l
          $ wrap_k (fmt ":@,(") (str ")")
              ( fmt_if islocal "local_ " $ fmt_pattern c ?parens xpat
              $ fmt " =@;<1 2>" $ fmt_expression c xexp ) )
...
and fmt_class_field c ctx cf =
  ...
  in
  let fmt_atrs = fmt_attributes c ~pre:(Break (1, 0)) ~key:"@@" atrs in
  (fun k ->
    fmt_cmts_before
    $ hvbox 0 ~name:"clf"
