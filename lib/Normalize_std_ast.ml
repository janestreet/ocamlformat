...
let equal fragment ?(debug = false) ~ignore_doc_comments ~erase_jane_syntax c
    ~old:ast1 ~new_:ast2 =
  let map = ast fragment c ~ignore_doc_comments in
  ...
  if (not result) && debug then (
    ...
    Out_channel.write_all p
      ~data:
        (Format.asprintf "OLD:\n %a\n\n"
           (Std_ast.Printast.ast fragment)
           ast1 ) ;
    Out_channel.write_all n
      ~data:
        (Format.asprintf "NEW:\n %a\n\n"
           (Std_ast.Printast.ast fragment)
           ast2 ) ;
    ignore (Stdlib.Sys.command (Format.sprintf "patdiff %s %s >&2" p n)) ;
    Stdlib.Sys.remove p ;
    Stdlib.Sys.remove n ) ;
  result
