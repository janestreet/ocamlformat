...
let sugar_cl_fun c xexp =
  ...
  in
  sugar_fun_ ~will_keep_first_ast_node:true xexp
...
and fmt_core_type c ?(box = true) ?(in_type_declaration = false)
    ?(need_space = false) ?pro ({ast= typ} as xtyp) =
  protect (Typ typ)
  @@
  ...
  | Ptyp_arrow _, Some _ when c.conf.ocp_indent_compat ->
      fmt_or need_space "@;" "@,"
...
and fmt_pattern c ?pro ?parens ({ctx= ctx0; ast= pat} as xpat) =
  ...
  | Ppat_array [] ->
      hvbox 0
        (wrap_fits_breaks_and_space "[|" "|]"
           (Cmts.fmt_within c.cmts ppat_loc))
  | Ppat_array pats ->
      hvbox 0
        (wrap "[| " "@ |]"
  ...
  | Ppat_unpack {txt} -> wrap_if parens "(" ")" (fmt "module@ " $ str txt)
  | Ppat_exception pat ->
      cbox 2
        (wrap_if parens "(" ")"
      ...
      | (lbl, ({pexp_desc= Pexp_fun _; pexp_loc} as eN1)) :: rev_e1N
        when List.for_all rev_e1N ~f:(fun (_, eI) ->
          ...
          hvbox 0
            ...
            $ fmt_atrs )
      | ( lbl
        , ( {pexp_desc= Pexp_function [{pc_lhs; pc_guard= None; pc_rhs}]} as
          eN ) )
        :: rev_e1N
        when List.for_all rev_e1N ~f:(fun (_, eI) ->
                 is_simple c.conf (fun _ -> 0) (sub_exp ~ctx eI) ) ->
...
and fmt_class_field c ctx (cf : class_field) =
  ...
  let fmt_kind = function
    ...
    | Cfk_concrete (_, {pexp_desc= Pexp_poly (e, poly)}) ->
        let xargs, xbody =
          match poly with
          | None -> sugar_fun c (sub_exp ~ctx e)
  ...
  hvbox 0
    ...
    $ hvbox 2
        ...
        $ Cmts.fmt c.cmts loc
            (wrap_if (is_symbol_id txt) "( " " )" (str txt))
        $ fmt_core_type c ~pro:":"
            ~box:(not (is_arrow_or_poly pval_type))
            ~need_space:true (sub_typ ~ctx pval_type)
...
and fmt_class_exprs c ctx (cls : class_expr class_infos list) =
  list_fl cls (fun ~first ~last:_ cl ->
      ...
      let xargs, xbody =
        ...
        | [] -> sugar_cl_fun c (sub_cl ~ctx pci_expr)
...
and fmt_with_constraint c ctx = function
  | Pwith_type ({txt; loc}, td) ->
      Cmts.fmt c.cmts loc @@ fmt " type "
...
and fmt_structure_item c ~last:_ ?ext {ctx; ast= si} =
  protect (Str si)
  @@
          ...
          let fmt_cstr, xbody =
            ...
            | Pexp_constraint (exp, typ), _ ->
                ( Some
                    ( fits_breaks " " "@;<1000 0>"
                    $ cbox 0
                        (fmt_core_type c ~pro:":" ~need_space:false
                           ~box:false (sub_typ ~ctx typ)) )
                , sub_exp ~ctx exp )
            | _ -> (None, xbody)
          in
