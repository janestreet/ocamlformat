...
let rec sugar_functor_type c ~allow_attributes ({ast= mty} as xmty) =
  ...
  | { pmty_desc= Pmty_functor (arg, arg_mty, body)
    ; pmty_loc
    ; pmty_attributes }
    when match pmty_attributes with
      | [] -> true | _ -> allow_attributes ->
    let arg =
        ...
        hvbox 1
          ...
          $ list_pn lines (fun ?prev curr ?next ->
                ...
                $ opt next (fun next ->
                      ...
                      in
                      fmt "\\n"
                      $ fmt_if_k (not (String.is_empty next))
...
and fmt_core_type c ?(box= true) ?pro ?(need_space=false) ({ast= typ} as xtyp) =
  ...
  | Ptyp_variant (rfs, flag, lbls) ->
      let row_fields rfs =
        match rfs, flag, lbls with
        | [(Rinherit _ as r)], Closed, None -> fmt "@ | " $ fmt_row_field c ctx r
        | rfs,_,_ -> list rfs "@ | " (fmt_row_field c ctx)
      in
      let protect_token =
...
and fmt_pattern c ?pro ?parens ({ctx= ctx0; ast= pat} as xpat) =
  ...
  | Ppat_unpack {txt} ->
      wrap_fits_breaks_if_no_space parens "(" ")" (fmt "module@ " $ str txt)
  | Ppat_exception pat ->
      cbox 2
        (wrap_if parens "(" ")"
...
and fmt_fun_args c args =
  let fmt_fun_arg = function
    ...
    | Val
      ...
      Cmts.fmt c.cmts loc
        @@ cbox 0
             (fmt "?(" $ str l $ equal $ fmt_expression c ?parens xexp $ fmt ")")
  ...
  fmt_if_k
    ...
    (list args "@;" (fun x -> hovbox 0 (fmt_fun_arg x)))
        ...
        list_pn args (fun ?prev (lbl, arg) ?next ->
            ...
            let openbox =
              fmt_if_k (Option.is_none prev)
                (open_hvbox (if first_grp then 2 else 0))
            in
...
and fmt_class_expr c ?eol ?(box= true) ({ast= exp} as xexp) =
  ...
  | Pcl_fun (_, _, p, _) ->
      ...
      hvbox_if box
        ...
        ( fmt_if parens "("
        $ ( open_hovbox 2
          $ (hovbox 4 (fmt "fun " $ fmt_fun_args c xargs $ fmt "@ ") $ fmt "->")
          $ close_box $ fmt "@ "
  ...
  let fmt_kind = function
    ...
    | Cfk_concrete (_, {pexp_desc= Pexp_poly (e, poly)}) ->
        ...
        let ty, e =
          ...
          | _, poly -> (poly, xbody)
        in
        fmt_fun_args c xargs $ fmt_if (not (List.is_empty xargs)) "@ "
  ...
  @@ fmt_cmts
       ...
       | Pcf_method (name, priv, kind) ->
           hovbox 2
             ...
             $ fmt "@ " $ str name.txt $ fmt "@ " $ fmt_kind kind )
...
and fmt_module_declaration c ctx ~rec_flag ~first pmd =
  ...
  in
  let xargs, xmty = sugar_functor_type c ~allow_attributes:false (sub_mty ~ctx pmd_type) in
  let colon =
          ...
          let fmt_cstr, xbody =
            ...
            | Pexp_constraint (exp, typ) ->
                ( Some
                    (fmt "@,"
                     $ cbox 0 (
                       fmt_core_type ~pro:":" ~need_space:false ~box:false c (sub_typ ~ctx typ)
                       $ fmt "@ "))
                , sub_exp ~ctx exp )
            | _ -> (None, xbody)
          in
