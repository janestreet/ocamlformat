...
let map_use_file mapper use_file =
  ...
  List.map use_file ~f:(fun toplevel_phrase ->
    ...
    | Parsetree.Ptop_def structure -> Ptop_def (mapper.Ast_mapper.structure mapper structure)
    | Parsetree.Ptop_dir ((dir : string), directive_argument) ->
      let directive_argument =
        match directive_argument with
        | Parsetree.Pdir_none
        | Parsetree.Pdir_string (_ : string)
        | Parsetree.Pdir_int ((_ : string), (_: char option))
        | Parsetree.Pdir_ident (_ : Longident.t)
        | Parsetree.Pdir_bool (_ : bool) -> directive_argument
      in
