...
let init_impl = init map_structure (List.map ~f:(fun {Parsetree.pstr_loc} -> pstr_loc))

let init_intf = init map_signature (List.map ~f:(fun {Parsetree.psig_loc} -> psig_loc))

let init_use_file =
  init Migrate_ast.map_use_file (List.concat_map ~f:(fun toplevel_phrase ->
