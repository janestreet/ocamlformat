...
let map_type_var sub (n, l) =
  map_loc sub n, map_opt (sub.jkind_annotation sub) l
...
module T = struct
  ...
  let map sub {ptyp_desc = desc; ptyp_loc = loc; ptyp_attributes = attrs; ptyp_loc_stack = _} =
    ...
    | Ptyp_unboxed_tuple tyl ->
        unboxed_tuple ~loc ~attrs (List.map (fun (lbl, t) -> map_opt (map_loc sub) lbl, sub.typ sub t) tyl)
    | Ptyp_constr (lid, tl) ->
  ...
  let map_type_declaration sub
    ...
    let params =
      List.map
        ...
        ptype_params
    in
    let jkind = map_opt (sub.jkind_annotation sub) ptype_jkind_annotation in
...
module PVB = struct
  let map_value_bindings sub
    { pvbs_bindings; pvbs_mutable; pvbs_rec; pvbs_extension } =
