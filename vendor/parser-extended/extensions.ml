...
module Comprehensions = struct
  ...
  *)
  ...
  let clause_binding_of_lb { lb_pattern; lb_expression; lb_attributes; _ } =
    { pattern = lb_pattern
    ; iterator = iterator_of_expr lb_expression
  ...
  let rec raw_comprehension_of_expr expr =
    match expand_comprehension_extension_expr expr with
    | ["for"], { pexp_desc = Pexp_let({ lbs_rec = Nonrecursive;
                                        lbs_bindings = iterators },
                                      rest); _ } ->
        add_clause
          (For (List.map clause_binding_of_lb iterators))
